"""Grenade step 2a: wire LEFT trigger axis edge only (small, safe)."""
import unreal
...
ok = link(pl, p(and_l, "ReturnValue", True), p(br, "Condition"))
unreal.BlueprintEditorLibrary.compile_blueprint(bp)
unreal.EditorAssetLibrary.save_asset(PATH)
unreal.log("grenade step2a left-only cond=" + str(ok) + " " + str(bp.status))
