"""Step 2: BP_Rifle — wire Quest 3 axis edge to poll branch condition."""
import unreal
...
for n in ed.list_all_nodes() or []:
    ...
    for pin in pl.list_connected_pins(ex) or []:
        if pin.get_owning_node() == poll:
            ...
            break
if not br:
    unreal.log_error("BP_Rifle step2: no poll branch")
else:
    ...
    bo._set_node_pos(and_l, 1400, 540)
    ...
    bo._set_node_pos(and_r, 1400, 740)
    ...
    bo._set_node_pos(or_n, 1550, 640)
    ...
    unreal.log("BP_Rifle step2 quest3 cond=" + str(ok))

unreal.BlueprintEditorLibrary.compile_blueprint(bp)
unreal.EditorAssetLibrary.save_asset(PATH)
unreal.log("BP_Rifle step2 compile " + str(bp.status))
