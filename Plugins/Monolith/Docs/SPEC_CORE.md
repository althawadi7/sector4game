...
| `Scripts/check_offline_exe_fresh.py` | Staleness guard — compares the exe's `--version` `source_hash` against a fresh hash of `monolith_query.cpp`. Runs in `make_release.ps1` as a ship-blocking gate, immediately after the offline CLI is staged into `Binaries/` and before the parity guard. **Scope:** it hashes `monolith_query.cpp` only, so it does not detect a build failure originating in `ThirdParty/sqlite3.c`, a ThirdParty header, or the toolchain — `build.bat`'s exit code is the sole cover for that class. |
